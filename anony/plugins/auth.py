...
async def _auth(_, m: types.Message):
    if m.chat.type == enums.ChatType.PRIVATE:
        return await m.reply_text(
            "This command works in groups and supergroups only, because it manages who can control the current chat."
        )
...
async def _authlist(_, m: types.Message):
    if m.chat.type == enums.ChatType.PRIVATE:
        return await m.reply_text(
            "This command works in groups and supergroups only, because it lists authorized users for the current chat."
        )
...
@app.on_message(filters.command(["admincache", "reload"]) & ~app.bl_users)
@lang.language()
async def _admincache(_, m: types.Message):
    if m.chat.type == enums.ChatType.PRIVATE:
        return await m.reply_text(
            "This command works in groups and supergroups only, because it refreshes the admin cache for the current chat."
        )
